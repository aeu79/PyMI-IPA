...
def count_alignment(encoded_focus_alignment, theta, q):
    ...
    range_height = range(alignment_height)
    range_width = range(alignment_width)
    for j in range_height:
    ...
    range_height = range(int(alignment_height))
    range_width = range(int(alignment_width))
    for l in range_height:
        for e in range_width:
    ...
    range_q_dos = range(q)
    for i in range_width:
        for alpha in range_q:
            for beta in range_q_dos:
...
if __name__ == "__main__":
    #fi= os.sys.argv[1]
    fi = "encoded_focus_alignment.npy"
    import cProfile
